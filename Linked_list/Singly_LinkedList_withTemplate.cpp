...
void linkedlist<t>::insert_end()
{
    ...
    do{
     ...
     node<t> *ptr = new node<t>; //new node which will be appended.
     ptr->data = val;
     ...
     {
         node<t> *temp; //temp node to help us travel to the last node in the list.
         temp = head;
...
void linkedlist<t>::insert_at() {
    ...
    else
    {
        ...
        while(flag != pos-1 || temp->next != NULL)
        {
            temp = temp->next;
